...
const SCAN_STEPS = [
  "Resolving channel identity & account age...",
  "Fetching live stream & viewer count...",
  "Pulling follower list & timestamp clustering...",
  "Scraping live chatters from channel...",
  "Running username entropy analysis...",
  "Fetching VODs, clips & subscription data...",
  "Collecting moderator & community signals...",
  "Computing engagement & ghost-viewer ratios...",
  "Sending real data to forensic AI engine...",
  "Compiling verified report...",
];
    ...
    <>
      ...
      <div className="app">
          ...
          <>
              ...
              <div>
                <div className="result-channel">
                  ...
                  <span>
                    ...
                    {result.dataQuality && ` · ${result.dataQuality.dataPointsCollected} DATA SOURCES`}
                  </span>
                </div>
                {result.usedRealData && (
                  <div className="real-data-badge">
                    <span className="dot" />
                    {result.dataQuality?.chattersSource === "api"
                      ? `LIVE API · STREAM · CHATTERS(${result.dataQuality?.chattersTotal}) · ${[result.dataQuality?.hasVODs && "VODS", result.dataQuality?.hasClips && "CLIPS", result.dataQuality?.hasSubs && "SUBS"].filter(Boolean).join(" · ")}`
                      : result.dataQuality?.chattersSource === "follower_fallback"
                      ? "⚠ NO CHAT TOKEN · SET TWITCH_USER_TOKEN FOR FULL DATA"
                      : "LIVE TWITCH API · OFFLINE CHANNEL"
                    }
                  </div>
                )}
              </div>
            ...
            <div className="panel" style={{ marginBottom: 16 }}>
              <div className="panel-title">
                Username Analysis
                {result.dataQuality?.chattersSource === "api"
                  ? <span style={{ fontSize: 10, color: "var(--accent)", marginLeft: 10, fontWeight: 400 }}>● LIVE CHATTERS — REAL DATA</span>
                  : <span style={{ fontSize: 10, color: "var(--warn)", marginLeft: 10, fontWeight: 400 }}>⚠ SHOWING RECENT FOLLOWERS (add TWITCH_USER_TOKEN for live chatters)</span>
                }
              </div>
                  ...
                  <tbody>
                    {result.chatSample?.map((u, i) => (
                      <tr key={i}>
                        ...
                        <td style={{ maxWidth: 240, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--text-dim)", fontSize: 11 }}>{u.reason}</td>
        ...
        <div className="disclaimer">
          <strong>⚠ DATA TRANSPARENCY:</strong> Risk score, engagement rate, username bot scores, follow-spike detection, VOD consistency, and all numeric metrics are computed algorithmically from real Twitch API data — no AI involvement. The <strong>verdict text and signal descriptions</strong> are written by an AI model interpreting those real numbers. The <strong>username table</strong> shows real chatter/follower usernames with algorithmic bot-pattern scores — no fabricated messages or account ages. Twitch does not expose individual message history or account creation dates via public API.
        </div>
